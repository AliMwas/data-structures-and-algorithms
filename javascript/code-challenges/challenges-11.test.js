...
const totalSum = (input) => {
  // Solution code here...
  let results = 0;
  input.map(arr => results += arr.reduce((acc, num) => acc += num, 0));
  return results;
};
...
let findShortest = (data) => {
  // Solution code here...
  let heights = data.map(z => z.height);
  heights.sort((a,b) => a-b);
  let answers = data.filter(i => i.height.includes(heights[0]));
